 ...
 * 
 * @author Steven Okamoto and Tal Grinshpoun
 *
 */
@Register(name="dcop-meeting-scheduling")
public class MeetingSchedulingDCOPGen extends AbstractSeedableProbGen {
	 ...
	 * conflicting meetings. 
	 * <li><code>MIN_IMPORTANCE</code> The importance of the least important meeting of the two.
	 * @author Steven (Tal added MIN_IMPORTANCE)
	 *
	 */
	public enum ConflictCost { BINARY, SUM_TOTAL, SUM_CONFLICT, MIN_TOTAL, MIN_IMPORTANCE };
	...
	@Variable(name="min-meeting-importance", description="minimum importance for a meeting", defaultValue="0")
	protected int minMeetingImportance = 0;
	@Variable(name="meeting-importance-range", description="number of possible values for a meeting importance", defaultValue="1")
	protected int meetingImportanceRange = 1;
	/**
	 * Added fields for considering the importance of a meeting
	 * @author Tal
	 *
	 */
	 ...
	 *     <li>"MIN_IMPORTANCE": The importance of the least important meeting of the two</li> 
	 * </ol>
	 */
				...
				if (meeting1.getNumOverlap(meeting2) > 0) {
					for (int time1 :domains.get(i)) {
						...
						for (int time2 : domains.get(j)) {
							...
							if (totalCost > 0) {
								prob.setConstraintCost(i, time1, j, time2, totalCost);
								prob.setConstraintCost(j, time2, i, time1, totalCost);
							}
						}
					}
				}
	...
	protected int computeConflictCost(Meeting m1, Meeting m2) {
		...
		case MIN_IMPORTANCE:
			// the importance of the least important meeting of the two
			cost = m1.getImportance() <= m2.getImportance() ? m1.getImportance() : m2.getImportance();
			break;
		default:
	 ...
	 */
	protected void initMeetingsByLoad(Meeting [] meetings, Random rand) {
	 ...
	 */
	protected int valueInRange(Random rand, int min, int range) {
	 ...
	 * 
	 * Modifications:
	 * 1. Changed Meeting to a protected class
	 * 2. Changed participants field to protected
	 * 3. Added an importance field, including a getter method for it
	 * @author Tal
	 *
	 */
	protected class Meeting {
		 ...
		 */
		protected List<Integer> participants;
		/**
		...
		public Meeting(Random rand) {
			...
			importance = minMeetingImportance + rand.nextInt(meetingImportanceRange);
		}
		...
		/**
		 * Gets the importance the meeting.
		 * @return The importance of the meeting.
		 * @author Tal
		 */
